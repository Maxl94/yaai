...
async def test_sa_auto_granted_access_on_create(
...
async def test_sa_with_access_can_read_model(
    owner_client: AsyncClient, pg_session_factory: async_sessionmaker
):
...
async def test_sa_without_access_cannot_read_model(
    owner_client: AsyncClient, pg_session_factory: async_sessionmaker
):
...
async def test_sa_sees_only_granted_models(
    owner_client: AsyncClient, pg_session_factory: async_sessionmaker
):
...
async def test_sa_with_no_access_sees_empty(
    owner_client: AsyncClient, pg_session_factory: async_sessionmaker
):
...
async def test_sa_with_both_sees_both(
    owner_client: AsyncClient, pg_session_factory: async_sessionmaker
):
...
async def test_owner_can_update_model(owner_client: AsyncClient):
    ...
    resp = await owner_client.put(
        f"/api/v1/models/{model['id']}", json={"name": "updated-model"}
    )
    assert resp.status_code == 200
...
async def test_viewer_cannot_update_model(owner_client: AsyncClient, viewer_client: AsyncClient):
    ...
    resp = await viewer_client.put(
        f"/api/v1/models/{model['id']}", json={"name": "hacked"}
    )
    assert resp.status_code == 403


async def test_sa_with_access_can_update_model(
    owner_client: AsyncClient, pg_session_factory: async_sessionmaker
):
    ...
    async with await _make_sa_client(pg_session_factory, raw_key) as sa_client:
        resp = await sa_client.put(
            f"/api/v1/models/{model['id']}", json={"name": "sa-updated"}
        )
        assert resp.status_code == 200


async def test_sa_without_access_cannot_update_model(
    owner_client: AsyncClient, pg_session_factory: async_sessionmaker
):
...
async def test_sa_cannot_delete_model(
    owner_client: AsyncClient, pg_session_factory: async_sessionmaker
):
...
async def test_sa_with_access_can_create_version(
    owner_client: AsyncClient, pg_session_factory: async_sessionmaker
):
...
async def test_sa_with_access_can_read_version(
    owner_client: AsyncClient, pg_session_factory: async_sessionmaker
):
...
async def test_sa_without_access_cannot_read_version(
    owner_client: AsyncClient, pg_session_factory: async_sessionmaker
):
...
async def test_owner_can_log_inference(owner_client: AsyncClient):
    ...
    version = await _create_version(owner_client, model["id"])
    resp = await owner_client.post(
        "/api/v1/inferences", json=_inference_payload(version["id"])
    )
    assert resp.status_code == 201
...
async def test_sa_with_access_can_log_inference(
    owner_client: AsyncClient, pg_session_factory: async_sessionmaker
):
...
async def test_grant_then_access(
    owner_client: AsyncClient, pg_session_factory: async_sessionmaker
):
...
async def test_revoke_then_denied(
    owner_client: AsyncClient, pg_session_factory: async_sessionmaker
):
    ...
    async with await _make_sa_client(pg_session_factory, raw_key) as sa_client:
        ...
        revoke_resp = await owner_client.delete(
            f"/api/v1/auth/models/{model['id']}/access/{sa_id}"
        )
...
async def test_grant_second_model(
    owner_client: AsyncClient, pg_session_factory: async_sessionmaker
):
...
async def test_revoke_one_of_two(
    owner_client: AsyncClient, pg_session_factory: async_sessionmaker
):
...
async def test_sa1_cannot_access_sa2_model(
    owner_client: AsyncClient, pg_session_factory: async_sessionmaker
):
...
async def test_sa2_cannot_access_sa1_model(
    owner_client: AsyncClient, pg_session_factory: async_sessionmaker
):
...
async def test_sa1_cannot_write_sa2_model(
    owner_client: AsyncClient, pg_session_factory: async_sessionmaker
):
    ...
    async with await _make_sa_client(pg_session_factory, sa1_key) as sa1_client:
        resp = await sa1_client.put(
            f"/api/v1/models/{model_b['id']}", json={"name": "hacked"}
        )
        assert resp.status_code == 403


async def test_sa1_list_excludes_sa2_models(
    owner_client: AsyncClient, pg_session_factory: async_sessionmaker
):
...
async def test_sa2_list_excludes_sa1_models(
    owner_client: AsyncClient, pg_session_factory: async_sessionmaker
):
...
async def test_sa_cannot_create_sa(
    owner_client: AsyncClient, pg_session_factory: async_sessionmaker
):
...
async def test_sa_cannot_grant_access(
    owner_client: AsyncClient, pg_session_factory: async_sessionmaker
):
